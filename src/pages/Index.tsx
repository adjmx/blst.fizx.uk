...
// ── GRASP / NIP-34 detection (NIP-11 supported_nips includes 34) ─────────────
const GRASP_CACHE = new Map<string, boolean>();
const GRASP_PENDING = new Map<string, Promise<boolean>>();
function probeGrasp(wsUrl: string): Promise<boolean> {
  if (GRASP_CACHE.has(wsUrl)) return Promise.resolve(GRASP_CACHE.get(wsUrl)!);
  const existing = GRASP_PENDING.get(wsUrl);
  if (existing) return existing;
  const httpUrl = wsUrl.replace(/^wss:\/\//, 'https://').replace(/^ws:\/\//, 'http://');
  const p = fetch(httpUrl, { headers: { Accept: 'application/nostr+json' }, signal: AbortSignal.timeout(8000) })
    .then(r => r.ok ? r.json() : null)
    .then((info: { supported_nips?: number[] } | null) => {
      const ok = !!info && Array.isArray(info.supported_nips) && info.supported_nips.includes(34);
      GRASP_CACHE.set(wsUrl, ok);
      return ok;
    })
    .catch(() => { GRASP_CACHE.set(wsUrl, false); return false; })
    .finally(() => { GRASP_PENDING.delete(wsUrl); });
  GRASP_PENDING.set(wsUrl, p);
  return p;
}
function useGraspStatus(wsUrl: string): boolean {
  const [grasp, setGrasp] = useState<boolean>(() => GRASP_CACHE.get(wsUrl) ?? false);
  useEffect(() => {
    let cancelled = false;
    probeGrasp(wsUrl).then(v => { if (!cancelled) setGrasp(v); });
    return () => { cancelled = true; };
  }, [wsUrl]);
  return grasp;
}
function GraspBadge({ wsUrl, size = 'sm', className = '' }: { wsUrl: string; size?: 'xs' | 'sm'; className?: string }) {
  const isGrasp = useGraspStatus(wsUrl);
  if (!isGrasp) return null;
  const sizeCls = size === 'xs' ? 'text-[8px] px-1 py-0' : 'text-[9px] px-1.5 py-0.5';
  return (
    <a href="https://github.com/nostr-protocol/nips/blob/master/34.md"
       target="_blank" rel="noopener noreferrer"
       title="GRASP — relay accepts NIP-34 git events"
       className={`font-mono font-bold tracking-widest border border-[#a78bfa]/60 text-[#a78bfa] hover:border-[#a78bfa] hover:bg-[#a78bfa]/10 transition-colors ${sizeCls} ${className}`}>
      GRASP
    </a>
  );
}
...
// ── Config ────────────────────────────────────────────────────────────────────
const DEFAULT_TARGET_RELAY = 'wss://relay.fizx.uk';
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <nav className="border-b border-border px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <a href="https://github.com/adjmx/blst.fizx.uk" target="_blank" rel="noopener noreferrer" title="Source on GitHub" aria-label="Source on GitHub" className="shrink-0 text-muted-foreground/60 hover:text-primary transition-colors">
          ...
          <div className="bg-card border border-border px-4 py-3 mb-4 min-h-[110px] flex items-start gap-4 flex-wrap">
            <div className="flex-1 min-w-0">
              <div className="flex items-start gap-1.5">
                <AnimatedTitle accent="blst" rest="" from="#e879f9" to="#818cf8" suffixRgba="rgba(232,121,249,0.55)" fontSize="clamp(28px, 5vw, 40px)" />
                  ...
                  {sources.map(url => {
                    ...
                    return (
                      <div key={url} className="flex items-center gap-1.5 border border-border px-2 py-0.5">
                        ...
                        <GraspBadge wsUrl={url} size="xs" className="ml-1" />
                        {s && s.status !== 'off' && (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {editingTarget ? (
                  ...
                  <div className="flex items-center gap-1.5 border border-primary/30 px-2 py-1">
                    ...
                    <GraspBadge wsUrl={targetRelay} size="xs" className="ml-1" />
                    {!running && (
                      <button
              ...
              {sources.map(url => {
                ...
                return (
                  <div key={url} className="flex items-center gap-2">
                    ...
                    <GraspBadge wsUrl={url} size="xs" className="shrink-0" />
